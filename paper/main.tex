...
% 3. BYTEROVER DESIGN
% ============================================================
\section{\byterover{} Design}
\label{sec:design}

In this section, we introduce the \byterover{} architecture and its components in detail.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Maturity tiers}:
    ...
    \emph{draft} ($\iota < 35$), \emph{validated} ($35 \leq \iota < 85$),
    and \emph{core} ($\iota \geq 85$).
    A hysteresis gap prevents rapid oscillation between tiers.
...
\begin{lstlisting}[firstnumber=1]
{
  ...
  "summary": {"added": 0, "updated": 1, "merged": 0, "failed": 1}
}
\end{lstlisting}
...
A hierarchical symbol tree provides O(1) lookup from relative paths to knowledge entries,
with bidirectional reference tracking (forward links and backlinks).
The tree supports five symbol kinds:
\emph{Domain}~(1), \emph{Topic}~(2), \emph{Subtopic}~(3), \emph{Context}~(4), and \emph{Summary}~(5).
At session start, a lightweight overview of the tree structure (domain names, topic counts,
most recently updated entries) is injected into the agent's system prompt,
giving it ambient awareness of what knowledge exists without dumping full contents.


\subsection{Multi-Agent Coordination}
\label{sec:multi-agent}

When multiple agents share a workspace, the \contexttree{} becomes a coordination medium
without requiring explicit messaging infrastructure.

\subsubsection{Three-Layer Memory Architecture}

\byterover{} operates alongside two lightweight memory layers,
creating a three-layer architecture:

\begin{table}[h]
\centering
\small
\begin{tabular}{@{}p{2.2cm}p{2.5cm}p{2cm}p{2cm}p{3.5cm}@{}}
\toprule
\textbf{Layer} & \textbf{Source} & \textbf{Lifetime} & \textbf{Written By} & \textbf{Purpose} \\
\midrule
Context Tree & \byterover{} & Persistent, LLM-curated & Agent (sandbox) & Domain knowledge with structure \\
Workspace Memory & \texttt{MEMORY.md} & Persistent, manual & Agent or human & Durable preferences \& decisions \\
Daily Memory & \texttt{YYYY-MM-DD.md} & Append-only & Agent in session & Running session context \\
\bottomrule
\end{tabular}
\caption{The three-layer memory architecture.
  The \contexttree{} provides structured, LLM-curated domain knowledge;
  workspace memory holds durable preferences;
  daily memory captures session context.}
\label{tab:memory-layers}
\end{table}

\subsubsection{Isolation and Access Control}

By default, each agent has its own isolated \contexttree{}.
When agents share a workspace:
(1)~write access can be scoped to specific subtrees;
(2)~a sequential, deduplicated task queue eliminates write-write conflicts;
(3)~atomic writes ensure each operation completes fully or not at all.


\subsection{Implementation}
\label{sec:implementation}

\byterover{} is implemented as a modular three-layer architecture:

\begin{itemize}[leftmargin=*]
  \item \textbf{Knowledge Layer.}
    The \contexttree{} uses the local filesystem with markdown files.
    MiniSearch provides in-process full-text indexing.
    The query cache uses in-memory storage with TTL expiry
    and SHA-256 fingerprinting for invalidation.

  \item \textbf{Execution Layer.}
    Curation runs in an isolated sandbox with a \texttt{ToolsSDK} interface.
    The sequential task queue processes operations one at a time,
    eliminating concurrency issues without locking.
  ...
  \item \textbf{Integration Layer.}
    ...
    A CLI interface supports human inspection and manual curation.
\end{itemize}

The entire system runs locally with zero external dependencies.
Knowledge is stored as markdown files that are version-controllable (\texttt{git diff}-able),
human-readable, and portable (copy the directory to move knowledge).
...
and \textbf{(2)~LongMemEval}~\citep{wu2024longmemeval},
a large-scale stress-test benchmark with average context length exceeding 100K tokens,
used to evaluate scalability and memory retention stability over extended interaction horizons.

\paragraph{Baselines.}
We compare against five state-of-the-art memory architectures:
\begin{itemize}[leftmargin=*]
  \item \textbf{Full Context}: Feeds the entire conversation history into the LLM.
  \item \textbf{A-MEM}~\citep{xu2025amem}: Linked node graph with evolutionary LLM-based rewriting.
  \item \textbf{MemoryOS}~\citep{kang2025memoryos}: Hierarchical tiers (STM~$\to$~LTM) with rule-based promotion.
  \item \textbf{Nemori}~\citep{nan2025nemori}: Dual memory (episodic tree + semantic graph) with gradient-inspired modification.
  \item \textbf{MAGMA}~\citep{jiang2026magma}: Multi-relational graph with intent-guided traversal.
  \item \textbf{SimpleMem}~\citep{liu2026simplemem}: Hybrid index with on-the-fly synthesis.
\end{itemize}

All systems use \texttt{gpt-4o-mini} as the backbone LLM for fair comparison.

\paragraph{Metrics.}
Following standard evaluation protocols~\citep{zheng2023llmjudge},
we primarily use the LLM-as-a-Judge score (\texttt{gpt-4o-mini}, temperature~0.0)
to assess semantic accuracy.
For completeness, we also report token-level F1 and BLEU-1~\citep{papineni2002bleu}.
...
SimpleMem         & \todo{--} & \todo{--} & \todo{--} & \todo{--} & \todo{--} & \todo{--} \\
\midrule
\textbf{\byterover{} (ours)} & \todo{--} & \todo{--} & \todo{--} & \todo{--} & \todo{--} & \todo{--} \\
\bottomrule
\end{tabular}
\caption{Performance on the LoCoMo benchmark evaluated using LLM-as-a-Judge.
  Baseline results for Full Context, A-MEM, MemoryOS, Nemori, and MAGMA
  are from \citet{jiang2026magma}.
  \todo{Fill in ByteRover results after running benchmarks.}}
\label{tab:locomo}
\end{table}

\todo{Analysis paragraph: Explain why ByteRover's agent-native curation
helps with multi-hop reasoning (explicit relations provide multi-hop paths),
adversarial queries (provenance enables verification),
and temporal queries (structured timestamps in Raw Concept).}


\subsection{Generalization on LongMemEval}
...
\toprule
\textbf{Question Type} & \textbf{Full-context} & \textbf{MAGMA} & \textbf{\byterover{} (ours)} \\
 & (101K tokens) & (0.7--4.2K tokens) & \todo{(tokens)} \\
\midrule
single-session-preference & 6.7\% & 73.3\% & \todo{--} \\
single-session-assistant & 89.3\% & 83.9\% & \todo{--} \\
temporal-reasoning & 42.1\% & 45.1\% & \todo{--} \\
multi-session & 38.3\% & 50.4\% & \todo{--} \\
knowledge-update & 78.2\% & 66.7\% & \todo{--} \\
single-session-user & 78.6\% & 72.9\% & \todo{--} \\
\midrule
\textbf{Average} & 55.0\% & 61.2\% & \todo{--} \\
\bottomrule
\end{tabular}
\caption{Performance comparison on LongMemEval.
  Baseline results from \citet{jiang2026magma} and \citet{jiang2026anatomy}.
  \todo{Fill in ByteRover results after running benchmarks.}}
\label{tab:longmemeval}
\end{table}

\todo{Analysis paragraph: Discuss token efficiency---ByteRover resolves
most queries at Tier~0--2 without LLM calls, achieving competitive accuracy
with substantially fewer tokens than Full Context.}
