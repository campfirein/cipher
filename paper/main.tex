...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[layer=gray!8, minimum width=8.0cm, minimum height=1.0cm]
      (daemon) at (0,6.0) {};
    \node[font=\scriptsize\bfseries, black!40, anchor=west] at (-3.8,6.0) {Daemon};
    \node[box, fill=white, minimum width=2.6cm] (tq) at (-1.2,6.0)
      {Task Queue};
    \node[box, fill=white, minimum width=2.6cm] (pool) at (1.8,6.0)
      {Agent Pool};

    % Client to daemon arrows
    \draw[arr, gray!50] (cli-tui.south) -- (cli-tui.south |- daemon.north);
    \draw[arr, gray!50] (cli-cli.south) -- (cli-cli.south |- daemon.north);
    \draw[arr, gray!50] (cli-mcp.south) -- (cli-mcp.south |- daemon.north);
    \node[lbl, font=\tiny, anchor=west] at (3.3,6.6) {Socket.IO};

    % --- Agent Process boundary ---
    \draw[thick, rounded corners=6pt, blue!25]
      (-4.3,4.85) rectangle (4.3,-0.65);
    \node[font=\scriptsize\bfseries, blue!40, anchor=west] at (-4.1,4.65) {Agent Process};
    \node[font=\tiny, blue!25, anchor=east] at (4.1,4.65) {\itshape one per project};

    % Daemon to agent arrow
    \draw[arr, gray!50] (pool.south) -- (pool.south |- 0,4.85);
    ...
    % --- Knowledge Layer (y=0.3) ---
    \node[layer=yellow!10, minimum width=8.0cm, minimum height=1.4cm]
      (kl) at (0,0.3) {};
    \node[font=\scriptsize\bfseries, yellow!40!black] at (0,0.8) {Knowledge Layer};
    \node[box, fill=white, minimum width=2.2cm] (ct) at (-1.5,0.05)
      {Context Tree};
    \node[box, fill=white, minimum width=2.2cm] (ms) at (1.0,0.05)
      {MiniSearch};
    \node[box, fill=white, minimum width=1.4cm] (cache) at (3.0,0.05)
      {Cache};
    \node[font=\tiny, text=black!35] at (0,-0.5)
    ...
    % QueryExecutor → Context Tree, MiniSearch, Cache
    \draw[arr] ([xshift=-5pt]qexec.south) -- ++(0,-0.25) -| (ct.north);
    \draw[arr] (qexec.south) -- ++(0,-0.40) -| (ms.north);
    \draw[arr] ([xshift=5pt]qexec.south) -- ++(0,-0.55) -| (cache.north);
    % CurateExecutor → Context Tree, MiniSearch
    \draw[arr] ([xshift=-4pt]cexec.south) -- ++(0,-0.35) -| ([xshift=4pt]ct.north);
    \draw[arr] ([xshift=4pt]cexec.south) -- ++(0,-0.50) -| ([xshift=4pt]ms.north);
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    out/.style={draw, rounded corners=2pt, fill=green!10,
    ...
    \node[out] (t0y) at (3.6,-1.8) {Return};

    \node[step=blue!8] (t1) at (0,-2.7)
      {Tier 1: Fuzzy cache $\geq$0.6?};
    \node[out] (t1y) at (3.6,-2.7) {Return};
...
Both models run at temperature~0.0 with top-$k$\,=\,10 results.

\paragraph{Evaluation prompts.}
To enable direct, apple-to-apple comparison with Hindsight~\citep{latimer2025hindsight},
