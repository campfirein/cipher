...
export const mapToPushContexts = (params: MapToPushContextsParams): CogitPushContext[] => {
  const {addedFiles, deletedPaths, modifiedFiles} = params

  const addedContextFiles = addedFiles.map((file) => new CogitPushContext({
  ...
  const editedContextFiles = modifiedFiles.map((file) => new CogitPushContext({
      content: file.content,
      operation: 'edit',
      path: file.path,
      tags: file.tags,
  ...
  const deletedContextFiles = deletedPaths.map((deletedPath) => new CogitPushContext({
      content: '',
      operation: 'delete',
      path: deletedPath,
      tags: [],
