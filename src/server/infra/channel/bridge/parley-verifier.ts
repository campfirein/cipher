...
export type VerifyHandshakeResult =
  ...
  | {ok: false; reason: VerifyFailureReason; retriable?: boolean}
...
export async function verifyHandshakeAndPin(args: VerifyHandshakeArgs): Promise<VerifyHandshakeResult> {
  ...
  if (!Number.isFinite(tsMs)) {
    return {ok: false, reason: 'HANDSHAKE_TS_EXPIRED'}
  }

  const drift = Math.abs(tsMs - args.now.getTime())
  if (drift > args.clockSkewMs) {
    return {ok: false, reason: 'HANDSHAKE_TS_EXPIRED'}
  }
  ...
  if (installPubBytes.length !== 32) {
    return {ok: false, reason: 'INSTALL_CERT_INVALID'}
  }
  ...
  if (derivedPeerId !== args.transportPeerId) {
    return {ok: false, reason: 'TRANSPORT_IDENTITY_MISMATCH'}
  }

  // Step 4: install cert self-signature + subject_id == derivePeerId(public_key).
  if (env.handshake.install_cert.subject_id !== derivedPeerId) {
    return {ok: false, reason: 'INSTALL_CERT_INVALID'}
  }
  ...
  if (!verifyInstallCert(installCertPayload, installSig, installPubKey)) {
    return {ok: false, reason: 'INSTALL_CERT_INVALID'}
  }
  ...
  if (!verifyParleyHandshake(handshakeInner, handshakeSig, installPubKey)) {
    return {ok: false, reason: 'HANDSHAKE_SIG_INVALID'}
  }
  ...
  if (args.nonceLru.has(args.transportPeerId, env.handshake.nonce)) {
    return {ok: false, reason: 'HANDSHAKE_REPLAY'}
  }

  // Step 7: accept_modes gate. Tree-cert kind must be in the local
  // accept list. Cheap policy check, but ONLY now that the cert is
  // L1-authenticated.
  if (!args.acceptModes.includes(env.handshake.tree_cert.cert_kind)) {
    return {ok: false, reason: 'CERT_KIND_REJECTED_BY_POLICY'}
  }

  // Step 8: tree cert chain. In slice 9.3 we support peer-tree only;
  // ca-issued-tree verification lands in a later slice.
  if (env.handshake.tree_cert.cert_kind === 'peer-tree') {
    ...
    if (!chain.ok) {
      return {ok: false, reason: 'TREE_CERT_INVALID'}
    }
  } else {
    // ca-issued-tree path deferred — surface as unsupported for now.
    return {ok: false, reason: 'TREE_CERT_INVALID'}
  }
  ...
  if (canonicalize(env.request_auth.requester_cert) !== canonicalize(env.handshake.tree_cert)) {
    return {ok: false, reason: 'CERT_CHAIN_MISMATCH'}
  }
  ...
  const computedBodyHash = createHash('sha256')
    ...
    .digest('hex')
  if (computedBodyHash !== env.request_auth.body_hash) {
    return {ok: false, reason: 'REQUEST_BODY_HASH_MISMATCH'}
  }
  ...
  const treePubKey = createPublicKey({
    format: 'jwk',
    key: {crv: 'Ed25519', kty: 'OKP', x: Buffer.from(treePubBytes).toString('base64url')},
  })
  const {signature: reqAuthSig, ...reqAuthPayload} = env.request_auth
  if (!verifyRequestAuth(reqAuthPayload, reqAuthSig, treePubKey)) {
    return {ok: false, reason: 'REQUEST_AUTH_INVALID'}
  }

  // Step 11: TOFU policy. Lookup existing pin; apply tofu_policy.
  const existing = await args.tofuStore.get(args.transportPeerId)
  if (!existing && args.tofuPolicy === 'deny') {
    return {ok: false, reason: 'PEER_UNPINNED'}
  }
  ...
  if (!existing && args.tofuPolicy === 'prompt') {
    ...
    return {ok: false, reason: 'TOFU_PROMPT_NOT_IMPLEMENTED'}
  }
  ...
  const pinned = await args.tofuStore.upsertWithMerge(args.transportPeerId, (priorPeer) => ({
    display_handle: env.handshake.install_cert.display_handle,
  ...
  return {
    ...
    ok: true,
    pinned,
    requestEnvelopeHash: requestEnvelopeHash(env),
  }
}
