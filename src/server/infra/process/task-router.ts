...
export class TaskRouter {
  ...
  private async handleTaskCreate(data: TaskCreateRequest, clientId: string): Promise<TaskCreateResponse> {
    ...
    const executeMsg: TaskExecute = {
      ...
      // R-3 (PHASE-2.5-PLAN.md §3.4): forward the curate-log identifier
      // (cur-<timestamp>) generated by CurateLogHandler.onTaskCreate so the
      // executor can include it in the per-leaf `Reason` field. Only present
      // for curate tasks — runCreateHooks returns undefined for other types.
      ...(logId ? {logId} : {}),
      ...(projectPath ? {projectPath} : {}),
      taskId,
      type: data.type,
