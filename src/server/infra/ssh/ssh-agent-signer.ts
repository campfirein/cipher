...
import {parseSSHPrivateKey} from './ssh-key-parser.js'
...
export class SshAgentSigner {
  ...
  async sign(payload: string): Promise<SSHSignatureResult> {
    ...
    // 2. Build signed data (uses 7-byte magic WITH null terminator)
    const signedData = Buffer.concat([
      SSHSIG_SIGNED_DATA_MAGIC,
      sshString(NAMESPACE),
      sshString(''),
...
export async function tryGetSshAgentSigner(keyPath: string): Promise<null | SshAgentSigner> {
  ...
  try {
    ...
    const parsed = await parseSSHPrivateKey(keyPath).catch(() => null)
