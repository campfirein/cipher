 ...
 * Re-exports centralized types from core domain and provides utility functions
 */
import {
  CleanMessage,
  CleanSession,
 ...
 * @param session.messages - Array of message objects to normalize
 * @param session.timestamp - Session timestamp
 * @param session.title - Session title
 * @param session.metadata - Session metadata
 * @param session.workspacePaths - Optional pre-extracted workspace paths
...
export function normalizeClaudeSession(session: Record<string, unknown>, sessionType: SessionType = 'Claude'): CleanSession {
  ...
  // Combine tool_use and tool_result messages
  normalizedMessages = combineToolResults(normalizedMessages)

  // Add turn_id based on timestamp order
  normalizedMessages = addTurnIds(normalizedMessages)
  ...
  const workspacePaths = extractWorkspacePaths(normalizedMessages, session.metadata, existingPaths)
