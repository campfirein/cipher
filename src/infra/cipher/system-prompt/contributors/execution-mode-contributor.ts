...
export class ExecutionModeContributor implements ISystemPromptContributor {
  ...
  public async getContent(context: SystemPromptContext): Promise<string> {
    ...
    if (context.isJsonInputMode === true) {
      ...
      // Always include segmentation instructions in headless mode
...
### MANDATORY FIRST TASK: Conversation Episode Analysis
...
4. **Generate Episode Documentation File:**

   a. Get timestamp:
   \`\`\`javascript
   bash_exec({ command: "date +%Y%m%d-%H%M%S" })
   \`\`\`
...
Now proceed with the episode analysis and documentation before answering the user's prompt.
`
  }
