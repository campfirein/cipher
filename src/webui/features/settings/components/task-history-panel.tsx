...
export function TaskHistoryPanel() {
  ...
  const rows = useMemo(() => {
    if (!data) return []
    return buildSettingsRows(data.items).filter((row) => row.category === 'task-history')
  }, [data?.items])

  return (
    <SettingsSection
      action={isLoading ? <LoaderCircle className="text-muted-foreground mt-1 size-4 animate-spin" /> : undefined}
      description="How many task records are retained per project."
      error={isError ? error : undefined}
      errorFallback="Failed to load task history settings"
