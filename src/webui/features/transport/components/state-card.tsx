...
function ByteroverMark() {
  return (
    <div className="flex flex-col items-center gap-2">
      <img alt="Byterover" className="size-8" src={logoUrl} />
    </div>
  )
}
...
export function StatusPill({children, tone}: {children: ReactNode; tone: Tone}) {
  ...
  return (
    <Badge className={cn('h-auto gap-1.5 px-2 py-0.5 text-[11px]', toneClasses[tone])} variant="outline">
      <span className={cn('relative size-1.5 rounded-full', dotColor[tone])}>
        <span className={cn('absolute inset-0 animate-ping rounded-full opacity-50', dotColor[tone])} />
      </span>
      {children}
    </Badge>
  )
  ...
  return (
    <div className="bg-background fixed inset-0 flex items-center justify-center p-6">
      <div className="border-border bg-card text-card-foreground w-full max-w-md overflow-hidden rounded-xl border shadow-[0_1px_0_0_rgba(0,0,0,0.02),0_8px_28px_-10px_rgba(0,0,0,0.12)] dark:shadow-[0_1px_0_0_rgba(0,0,0,0.4),0_8px_32px_-10px_rgba(0,0,0,0.6)]">
        ...
        {footer && (
          <div className="border-border bg-foreground/1.5 flex items-center gap-2 border-t px-5 py-2.5">
            {footer}
          </div>
        )}
      </div>
