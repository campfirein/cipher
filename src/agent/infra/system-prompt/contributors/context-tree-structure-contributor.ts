...
export class ContextTreeStructureContributor implements SystemPromptContributor {
  ...
  private async buildContextTreeStructure(contextTreePath: string): Promise<string> {
    ...
    parts.push(
      ...
      '- Domain names should be descriptive, use snake_case (e.g., `architecture`, `market_trends`, `risk_analysis`)',
  ...
  private buildEmptyContextTreeMessage(): string {
    return [
      ...
      '- Choose semantically meaningful domain names that represent broad knowledge categories',
      '- Use snake_case format for domain names (e.g., `data_models`, `trading_strategies`, `personal_notes`)',
