...
export default class Clear extends Command {
  ...
  public static description = 'Clear local context tree managed by ByteRover CLI'
  ...
  protected async confirmClear(): Promise<boolean> {
    return confirm({
      default: false,
      message: 'Are you sure you want to clear the context tree? This action cannot be undone.',
    })
  }
  ...
  public async run(): Promise<void> {
    ...
    try {
      ...
      if (!exists) {
        this.log('No context found. Nothing to clear.')
        return
      }
      ...
      if (!flags.yes) {
        ...
        if (!confirmed) {
          this.log('Cancelled. Context tree was not cleared.')
          return
        }
      }
      ...
      this.log('✓ Context tree cleared successfully.')
    } catch (error) {
      ...
      this.error(error instanceof Error ? error.message : 'Failed to clear context tree')
    }
  }
}
