...
import type {CommandSuggestion, SlashCommand} from '../../../types/index.js'

import {useCommands} from './use-commands-controller.js'
...
export function useSlashCompletion(input: string): UseSlashCompletionReturn {
  ...
  const hasMatchedCommand = useMemo(() => {
    ...
    const parts = withoutSlash.split(/\s+/)
    const commandPart = parts[0]?.toLowerCase() ?? ''
    return commands.some((cmd) => cmd.name === commandPart)
