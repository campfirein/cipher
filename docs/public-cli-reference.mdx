...
| `brv curate`                                    | Add or update a bullet in the playbook       | `brv curate --interactive`                         |
...
- Create a fresh configuration and ACE playbook
...
- **Warning:** Re-initialization deletes all local playbook data, reflections, and executor outputs
...
| `-c, --context <value>`   | Context of the bullet                             | No\*     |
...
brv curate --interactive
brv curate -i

# Add new bullet directly
brv curate --section "Common Errors" --context "Auth fails when token expires"

# Update existing bullet
brv curate --section "Common Errors" --bullet-id "common-00001" --context "Updated: Auth fails when token expires"

# Short form
brv curate -s "Best Practices" -c "Always validate user input before processing"
```

**Notes**

- Playbook must exist (run `brv init` first)
- Interactive mode: prompts for section (with search), context, and confirmation
- Non-interactive mode: requires both `--section` and `--context` flags
