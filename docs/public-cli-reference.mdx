...
| Flag | Description | Default |
| ---- | ----------- | ------- |
| `-f, --files <paths>` | Include file or folder paths for context (max 5 total). Use multiple `-f` flags for multiple paths. Folders are packed into a structured format | None |
| `--headless` | Run in headless mode (no TTY required) | `false` |
| `--format <option>` | Output format: `text` or `json` | `text` |
...
# Include folder reference
brv curate "Authentication module overview" -f src/auth/
...
- **File and folder references**: Up to 5 references total. Use `--files` or `-f` flag for CLI mode, or `@path` syntax in REPL mode. For folders, contents are packed into a structured format. Files must be text files within the project directory
