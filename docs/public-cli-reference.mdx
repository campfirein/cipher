...
| `brv add [CONTENT]`                             | Add content to context tree                  | `brv add "JWT tokens expire in 24h"`               |
...
| Argument  | Type   | Required | Description                                            |
| --------- | ------ | -------- | ------------------------------------------------------ |
| `CONTENT` | string | No       | Content to add (triggers autonomous mode if provided) |
...
brv add "React components follow atomic design pattern"
```
...
- Use `--verbose` flag to see detailed LLM execution steps
