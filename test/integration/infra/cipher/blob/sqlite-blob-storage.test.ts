...
import {BlobError, BlobErrorCode, SqliteBlobStorage} from '../../../../../src/infra/cipher/blob/index.js'
...
describe('SqliteBlobStorage', () => {
  ...
  describe('store', () => {
    ...
    it('should throw error if total size would exceed limit', async () => {
      ...
      // Try to store another 900KB blob (would exceed 5MB total limit)
  ...
  describe('edge cases', () => {
    ...
    it('should preserve createdAt when updating blob', async () => {
      const key = 'preserve-created'
      const content1 = Buffer.from('original')
      const content2 = Buffer.from('updated')

      const first = await storage.store(key, content1)
      // Small delay to ensure timestamps differ
      await new Promise((resolve) => {
        setTimeout(resolve, 10)
      })
      const second = await storage.store(key, content2)

      expect(second.metadata.createdAt).to.equal(first.metadata.createdAt)
      expect(second.metadata.updatedAt).to.be.greaterThan(first.metadata.updatedAt)
    })
  })
})
