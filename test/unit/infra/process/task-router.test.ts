...
describe('TaskRouter', () => {
  ...
  describe('task:create', () => {
    ...
    describe('onTaskCreateRequest hook firing (M13.1)', () => {
      it('fires onTaskCreateRequest with the raw request and clientId once per inbound task:create', async () => {
        const onTaskCreateRequest = sandbox.stub().resolves()
        const routerWithHook = new TaskRouter({
          agentPool,
          getAgentForProject,
          lifecycleHooks: [{onTaskCreateRequest}],
          projectRegistry,
          projectRouter,
          transport: transportHelper.transport,
        })
        routerWithHook.setup()

        const handler = transportHelper.requestHandlers.get(TransportTaskEventNames.CREATE)
        const request = makeTaskCreateRequest()
        await handler!(request, 'client-1')

        expect(onTaskCreateRequest.calledOnce).to.equal(true)
        expect(onTaskCreateRequest.firstCall.args[0]).to.deep.equal(request)
        expect(onTaskCreateRequest.firstCall.args[1]).to.equal('client-1')
      })

      it('does NOT fire onTaskCreateRequest on a duplicate-taskId retry (idempotent invocation)', async () => {
        const onTaskCreateRequest = sandbox.stub().resolves()
        const routerWithHook = new TaskRouter({
          agentPool,
          getAgentForProject,
      ...
      it('still fires onTaskCreateRequest when the task type is invalid (CLI invocation happened regardless)', async () => {
        const onTaskCreateRequest = sandbox.stub().resolves()
        const routerWithHook = new TaskRouter({
          agentPool,
          getAgentForProject,
      ...
      it('swallows hook errors and continues to validate + dispatch the task', async () => {
        const onTaskCreateRequest = sandbox.stub().rejects(new Error('boom'))
        const routerWithHook = new TaskRouter({
          agentPool,
          getAgentForProject,
          lifecycleHooks: [{onTaskCreateRequest}],
