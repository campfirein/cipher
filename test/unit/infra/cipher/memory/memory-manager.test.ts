...
describe('Memory Module', () => {
  ...
  describe('attachBlob', () => {
    ...
    it('should attach multiple blobs to the same memory', async () => {
      ...
      // Attach first blob
      const attachment1 = await memoryManager.attachBlob(
        memory.id,
      ...
      // Attach second blob
      const attachment2 = await memoryManager.attachBlob(
        memory.id,
  ...
  describe('getAttachment', () => {
    it('should retrieve blob content', async () => {
      ...
      // Get the attachment
      const storedBlob = await memoryManager.getAttachment(
        memory.id,
        attachment.blobKey,
      );
  ...
  describe('integration scenarios', () => {
    it('should handle complete attachment lifecycle', async () => {
      ...
      const doc1 = await memoryManager.attachBlob(
        memory.id,
        Buffer.from('Meeting notes'),
        {name: 'meeting.txt', type: 'text/plain'},
      );

      const doc2 = await memoryManager.attachBlob(
        memory.id,
