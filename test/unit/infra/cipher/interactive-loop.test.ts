...
describe('interactive-loop', () => {
  ...
  beforeEach(async () => {
    ...
    ;(readline.createInterface as sinon.SinonStub).returns(mockReadline)
  ...
  describe('startInteractiveLoop()', () => {
    ...
    it('should display welcome message with default values', async () => {
      // Mock promptUser to return exit command immediately
      ...
      // Mock executeCommand to return false (exit)
      sandbox.stub(await import('../../../../src/infra/cipher/interactive-commands.js'), 'executeCommand').resolves(false)

      await startInteractiveLoop(mockAgent)
    ...
    it('should handle exit command and stop loop', async () => {
      ...
      // Wait for readline to be set up
      await setTimeout(35)
      setupHandler()
      if (lineHandler) {
        lineHandler('/exit')
    ...
    it('should cleanup resources in finally block', async () => {
      ...
      expect((process.off as sinon.SinonStub).calledWith('SIGINT', sinon.match.any)).to.be.true
      expect((process.off as sinon.SinonStub).calledWith('SIGTERM', sinon.match.any)).to.be.true
    })
  })
})
