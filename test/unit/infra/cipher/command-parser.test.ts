...
import { parseInput, type CommandResult } from '../../../../src/infra/cipher/command-parser.js'
    ...
    describe('parseInput()', () => {
        ...
        describe('quoted argument parsing', () => {
            ...
            it('should handle escaped quotes with double quotes', () => {
                const result = parseInput('/delete "session\\"name"')
            ...
            it('should handle escaped backslashes', () => {
                const result = parseInput('/cmd "path\\\\to\\\\file"')

                expect(result.type).to.equal('command')
                if (result.type === 'command') {
                    expect(result.command).to.equal('cmd')
                    expect(result.args).to.deep.equal(['path\\to\\file'])
                }
            })
