...
import {getGlobalConfigDir, getGlobalConfigPath, getGlobalDataDir} from '../../../src/utils/global-config-path.js'
...
describe('global-config-path', () => {
  ...
  describe('getGlobalDataDir()', () => {
    ...
    it('should return different path than getGlobalConfigDir() (different parent)', () => {
      const configDir = getGlobalConfigDir()
      const dataDir = getGlobalDataDir()

      // Both end with 'brv' but have different parent directories
      expect(dataDir).to.match(/brv$/)
      expect(configDir).to.match(/brv$/)
      // Parent directories should be different (config vs data)
      expect(dataDir).to.not.equal(configDir)
    })
